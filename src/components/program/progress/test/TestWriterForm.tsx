...
import { Multiple, Subjects, MutlipleOption } from "../../../../slice/program/programProgressExamSlice";
...
const TestWriterForm = ({ subtogglePopup }: Props) => {
  ...
  const handleQuestionOptionChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, index: number, alpha_index: number) => {
    const updatedTestType = { ...myTestType };
    const updatedQuestion = { ...updatedTestType.testType[index] }; // Create a copy of the question object
    ...
    <TestWriterform>
      ...
      {fields.map((field, index) => (
        <TestWriterlist >
            ...
            <TWselect>
              <TestSelect>
                <Button
                  ...
                  onClick={() => handleButtonClick(index, "MULTIPLE_CHOICE_QUESTION")}
                >
                  객관식
                </Button>
                <Button
          ...
          {myTestType.testType[index].questionType === "MULTIPLE_CHOICE_QUESTION" && (
            <TestWrite>
              <Testtext>
                <Input
                  name="question"
              ...
              </Testtext>
              {
                alphaList.map((alpha, alpha_index) => (
                  <TestNumber key={alpha_index}>
                    <p style={{ marginRight: "0.6rem", marginTop: "1rem" }}>{alpha}</p>
                    <Input
                      name="choices"
                      ...
                      onChange={(e) => handleQuestionOptionChange(e, index, alpha_index)}
                    ></Input>
                  </TestNumber>
                ))
              }
